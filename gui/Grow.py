...
class Grow_DB(QtWidgets.QMainWindow):
    ...
    def __init__(self, row_count, file_path, folder_path, partitions):
        ...
        self.process_data = uic.loadUi("C:/winutils/cpd/gui/Grow.ui")
    ...
    def DB_Create(self):
        ...
        list_origins = ["ASCARD", "RR", "SGA", "BSCS"]
        ...
        for brand in list_brands:
            ...
            if brand == "castigo":
                ...
                self.Save_File(RDD_Data, root, partitions, brand, origin)

                for position in range(4):

                    if position % 2 == 0:

                        origin_list = [origin_list[position], origin_list[position + 1]]
                        origin = f"{origin_list[position]} - {origin_list[position + 1]}"
                        origin_list = list_origins
                        origin = "Multiorigen"
                        RDD_Data = self.Function_Complete(file, brand, origin_list)
                        self.Save_File(RDD_Data, root, partitions, brand, origin)
    ...
    def Function_Complete(path, brand_filter, origin_filter):
        ...
        Data_Root = Data_Root.withColumn("57_", \
            when((Data_Root.Mod_init_cta <= 20000), lit("1 Menos a 20 mil")) \
                ...
                .when((Data_Root.Mod_init_cta <= 1000000), lit("8 Entre 500 mil a 1 Millon")) \
                .when((Data_Root.Mod_init_cta <= 2000000), lit("9 Entre 1 a 2 millones")) \
                .otherwise(lit("9.1 Mayor a 2 millones")))
        
        return Data_Root

    def Save_File(Data_Frame, Directory_to_Save, Partitions, Brand_Filter, Origin_Filter):

        now = datetime.now()
        Time_File = now.strftime("%Y%m%d_%H%M")
        
        if Brand_Filter == "castigo":
            Type_File = f"BASE_Planilla_{Brand_Filter}_{Origin_Filter}"
