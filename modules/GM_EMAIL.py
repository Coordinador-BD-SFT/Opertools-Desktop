...
def Order_Columns(Data_Frame):
    ...
    Data_Frame = Data_Frame.withColumn("A", lit("1"))
    Data_Frame = Data_Frame.withColumn("B", lit("CONTEO"))
    Data_Frame = Data_Frame.withColumn("C", lit("FORMULA"))

    Columns_Email = ["Canal", "Dato_Contacto", "descuento", \
                    "rango", "tipo_de_cliente", "Form_Moneda", "Nombre_Completo", \
                    "Referencia", "Cuenta", "A", "B", "C"]
    ...
    if widget_filter != "Tables":
        ...
        for i, file in enumerate(os.listdir(output_path), start=1):
            if file.endswith(".csv"):
                ...
                new_file_path = os.path.join(output_path, f'EMAIL Part- {i}.csv')
