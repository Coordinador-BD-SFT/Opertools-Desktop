...
from pyspark.sql.functions import col, concat, lit, upper, regexp_replace, concat_ws, to_date
    ...
    if widget_filter != "Tables":
        ...
        for i, file in enumerate(os.listdir(output_path), start=1):
            if file.endswith(".csv"):
                ...
                new_file_path = os.path.join(output_path, f'BOT {File_Date} Part- {i}.csv')
...
def ServiceBot(RDD, Type_Proccess):
    ...
    RDD_1 = RDD.select("identificacion", "phone1", "phone2", "phone3", "nombrecompleto", \
                     f"{Price_Col}", "fecha_vencimiento", "cuenta", "origen", "marca", "descuento", "cuenta2", "mejorperfil_mes")
...
def WiseBot(RDD, Type_Proccess):
    ...
    RDD = RDD.withColumn(
            "CAJA",
            when(col("origen") == "BSCS", "P009-4")
            .when(col("origen") == "ASCARD", "P009-7")
            .when(col("origen") == "RR", "P009-5")
            .when(col("origen") == "SGA", "P009-6")
            .otherwise(lit("P009-"))
        )

    RDD = RDD.select("NOMBRE CAMPANA", "identificacion", "nombrecompleto", "APELLIDO COMPLETO", "Dato_Contacto", f"{Price_Col}", \
                      "MARCA", "ESTADO", "DIAS MORA", "CAJA", "cuenta","cuenta2",  "descuento", "fecha_vencimiento", "origen", "mejorperfil_mes")
